...
int main()
{
    ...
    {
        if (strncmp(months[tm.tm_mon], __DATE__, 3) == 0)
            break;
    }
    tm.tm_year -= 1900;
    mktime(&tm);
...
static void SanityCheck(void)
{
    ...
    if (hour != tm.tm_hour)
        Error("Msecs2Time", "hour(%u ms) != %u", msec, tm.tm_hour);
    if (mins != tm.tm_min)
        Error("Msecs2Time", "mins(%u ms) != %u", msec, tm.tm_min);
    if (secs != tm.tm_sec)
        Error("Msecs2Time", "secs(%u ms) != %u", msec, tm.tm_sec);

    if (Time2Msecs(hour, mins, secs) != msec)
        Error("Time2Msecs", "%02u:%02u:%02u != %u ms", hour, mins, secs, msec);
    LEDs(1, 0);
}
